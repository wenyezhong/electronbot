...
#define USBD_CDC_CMD_INTERFACE_STRING_FS     "CDC CMD Interface"
#define USBD_CDC_DATA_INTERFACE_STRING_FS     "CDC DATA Interface"
...
uint8_t * USBD_UsrStrDescriptor(struct _USBD_HandleTypeDef *pdev, uint8_t index,  uint16_t *length)
{
 ...
 /*  if (USBD_IDX_MICROSOFT_DESC_STR == index) {
  ...
  if (USBD_IDX_CDC_CMD_STR == index) {
    USBD_GetString((uint8_t *)USBD_CDC_CMD_INTERFACE_STRING_FS, USBD_CMPSIT_StrDesc, length);
    return USBD_CMPSIT_StrDesc;
  }
  else if (USBD_IDX_CDC_DATA_STR == index) {    
    USBD_GetString((uint8_t *)USBD_CDC_DATA_INTERFACE_STRING_FS, USBD_CMPSIT_StrDesc, length);
    return USBD_CMPSIT_StrDesc;
  }    
...
static uint8_t *USBD_CMPSIT_GetFSCfgDesc(uint16_t *length)
{
  ...
  USBD_EpDescTypeDef *pCdcEpCmdDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_CMD_EP);
  USBD_EpDescTypeDef *pCdcEpOutDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_OUT_EP);
  USBD_EpDescTypeDef *pCdcEpInDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_IN_EP);

  if (pCdcEpCmdDesc != NULL)
  {
    pCdcEpCmdDesc->bInterval = CDC_FS_BINTERVAL;
  }

  if (pCdcEpOutDesc != NULL)
  {
    pCdcEpOutDesc->wMaxPacketSize = CDC_DATA_FS_MAX_PACKET_SIZE;
  }

  if (pCdcEpInDesc != NULL)
  {
    pCdcEpInDesc->wMaxPacketSize = CDC_DATA_FS_MAX_PACKET_SIZE;
  }
...
static uint8_t *USBD_CMPSIT_GetHSCfgDesc(uint16_t *length)
{
  ...
  if (pCdcEpOutDesc != NULL)
  {
    pCdcEpOutDesc->wMaxPacketSize = CDC_DATA_HS_MAX_PACKET_SIZE;
  }

  if (pCdcEpInDesc != NULL)
  {
    pCdcEpInDesc->wMaxPacketSize = CDC_DATA_HS_MAX_PACKET_SIZE;
  }
