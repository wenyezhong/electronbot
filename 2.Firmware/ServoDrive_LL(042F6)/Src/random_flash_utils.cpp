...
void eeprom_write_byte(uint32_t pos, uint8_t value)
{
    ...
    /* with actual EEPROM, pos is a relative address */
    if (pos <= (DATA_EEPROM_END - DATA_EEPROM_BASE)) {
      if (LL_FLASHEx_DATAEEPROM_Unlock() == LL_OK) {
        LL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_BYTE, (pos + DATA_EEPROM_BASE), (uint32_t)value);
        LL_FLASHEx_DATAEEPROM_Lock();
      }
    }
#else
