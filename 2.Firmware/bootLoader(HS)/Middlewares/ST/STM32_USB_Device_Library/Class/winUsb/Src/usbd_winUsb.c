...
uint8_t WinUsb_Transmit_FS(uint8_t* Buf, uint16_t Len)
{
  ...
  /* USER CODE BEGIN 7 */
  USBD_WinUsb_HandleTypeDef *hcdc = (USBD_WinUsb_HandleTypeDef*)hUsbDeviceHS.pClassData;
  if (hcdc->TxState != 0){
    return USBD_BUSY;
  }
  USBD_WinUsb_SetTxBuffer(&hUsbDeviceHS, Buf, Len);
  result = USBD_WinUsb_TransmitPacket(&hUsbDeviceHS);
  /* USER CODE END 7 */
  return result;
}
