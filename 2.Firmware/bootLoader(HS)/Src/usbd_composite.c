...
__ALIGN_BEGIN static uint8_t USBD_CMPSIT_CfgDesc[USB_CMPSIT_CONFIG_DESC_SIZ] __ALIGN_END =
{
  ...
  LOBYTE(WIN_DATA_FS_OUT_PACKET_SIZE),
  HIBYTE(WIN_DATA_FS_OUT_PACKET_SIZE),
  ...
  0x02,                                            /* Bulk endpoint type */
  LOBYTE(WIN_DATA_FS_IN_PACKET_SIZE),
  HIBYTE(WIN_DATA_FS_IN_PACKET_SIZE),
  0x00                                             /* Polling interval in milliseconds */
  //106
};
...
void USBD_CMPSIT_AddClass(USBD_HandleTypeDef *pdev, USBD_ClassTypeDef *pclass, USBD_CompositeClassTypeDef classtype, uint8_t *EpAddr)
{
	...
	switch(classtype)
	{
		...
		case CLASS_TYPE_WINUSB:{
			...
			pdev->tclasslist[pdev->classId].Eps[0].size = WIN_DATA_FS_OUT_PACKET_SIZE;
			pdev->tclasslist[pdev->classId].Eps[0].is_used = 1U;
			
			pdev->tclasslist[pdev->classId].Eps[1].add = WIN_USB_EPIN_ADDR;
			pdev->tclasslist[pdev->classId].Eps[1].type = USBD_EP_TYPE_BULK;
			pdev->tclasslist[pdev->classId].Eps[1].size = WIN_DATA_FS_IN_PACKET_SIZE;
			pdev->tclasslist[pdev->classId].Eps[1].is_used = 1U;
...
static uint8_t *USBD_CMPSIT_GetFSCfgDesc(uint16_t *length)
{
	USBD_EpDescTypeDef *pEpCmdDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_CMD_EP);
  USBD_EpDescTypeDef *pEpOutDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_OUT_EP);
  USBD_EpDescTypeDef *pEpInDesc = USBD_GetEpDesc(USBD_CMPSIT_CfgDesc, CDC_IN_EP);
